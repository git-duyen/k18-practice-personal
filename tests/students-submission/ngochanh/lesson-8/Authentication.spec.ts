...
test.describe("AUTH-Authentication", async () => {
    ...
    test("@AUTH_001: Login fail", async ({ page }) => {
        ...
        await test.step("Nhập vào thông tin username, password bị sai", async () => {
            ...
            await page.locator("//input[@id='user_login']").fill(usn);

            await page.locator("//input[@id='user_pass']").fill(pw);
        });

        await test.step("Click button login", async () => {
            await page.locator("//input[@id='wp-submit']").click();
            await expect(page.locator("//div[@id='login_error']")).toHaveText(`Error: The username ${usn} is not registered on this site. If you are unsure of your username, try your email address instead.`)
        });
    });
    ...
    test("@AUTH_002: Login success", async ({ page }) => {
        await test.step("Nhập vào thông tin username, password đúng", async () => {
            ...
            await page.locator("//input[@id='user_login']").fill(usn);
            await expect(page.locator("//input[@id='user_login']")).toHaveValue(usn);

            await page.locator("//input[@id='user_pass']").fill(pw);
            await expect(page.locator("//input[@id='user_pass']")).toHaveValue(pw);
        });
