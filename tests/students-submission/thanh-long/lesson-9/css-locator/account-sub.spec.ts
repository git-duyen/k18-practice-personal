...
test.describe('ACCOUNT - subscriber permission', () => {
    ...
    test('@ACCOUNT_001: Create account with sub permission', async ({ page }) => {
        await test.step('Create new account successfully', async () => {
            ...
            await page.locator('select#role').selectOption('subscriber');
        ...
        await test.step('Login out admin account and login with new account', async () => {
            ...
            await expect(page.locator('div:has-text("Dashboard")').first()).toBeVisible();
            await expect(page.locator('div:has-text("Profile")').first()).toBeVisible();
