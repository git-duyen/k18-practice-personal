...
test.describe('ACCOUNT - subscriber permission', () => {
    ...
    test('@ACCOUNT_001: Create account with editor permission', async ({ page }) => {
        await test.step('Create new account successfully', async () => {
            ...
            await page.getByRole('textbox', { name: 'Username (required)' }).fill('E101-ThanhLong-subscriber');
            await page.getByRole('textbox', { name: 'Email (required)' }).fill('thanhlong@subscriber.com');
            await page.getByRole('textbox', { name: 'First Name' }).fill('E101');
            await page.getByRole('textbox', { name: 'Last Name' }).fill('ThanhLong-subscriber');
        ...
        await test.step('Login out admin account and login with new account', async () => {
            ...
            await page.getByRole('textbox', { name: 'Username or Email Address' }).fill("thanhlong@subscriber.com");
            ...
            await expect(page.locator(`//div[normalize-space()='Posts']`)).toBeHidden();
            await expect(page.locator(`//div[normalize-space()='Media']`)).toBeHidden();
            await expect(page.locator(`//div[normalize-space()='Pages']`)).toBeHidden();
            await expect(page.locator(':text-is("Comments")')).toBeHidden();
            await expect(page.locator(`//div[normalize-space()='Tools']`)).toBeHidden();
