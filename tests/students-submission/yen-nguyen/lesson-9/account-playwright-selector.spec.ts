...
test.describe('ACCOUNT - Account with Playwright selector', () => {
  ...
  test.afterEach(async ({ page }) => {
    ...
    if (userToDelete !== null) {
      await test.step('Teardown: Delete created account by admin account', async () => {
  ...
  test('@ACC_001: Create account with editor permission', async ({ page }) => {
    ...
    await test.step('Step 1: Go to user management page', async () => {
      ...
      const addUserButton = page.getByRole('link', { name: /Add New|Add User/ }).first();

      await expect(addUserButton).toBeVisible();
    });

    await test.step('Step 2: Create new user with Editor role', async () => {
      await page.getByRole('link', { name: /Add New|Add User/ }).first().click();

      await expect(page).toHaveURL(/.*user-new\.php.*/);
      await expect(page.getByRole('heading', { name: /Add New User|Add User/ })).toBeVisible();

      await fillUserForm(page, newUser);
...
async function login(page: Page, username: string, password: string): Promise<void> {
  ...
  await page.getByLabel('Username or Email Address', { exact: true }).fill(username);
...
async function fillUserForm(page: Page, user: TestUser): Promise<void> {
...
async function deleteUserIfExists(page: Page, username: string): Promise<void> {
  ...
  try {
    ...
    const deleteLink = userRow.locator('a.submitdelete').first();

    if (await deleteLink.isVisible().catch(() => false)) {
      await deleteLink.click();
    } else {
      await userRow.locator('.row-actions .delete a').first().click();
    }
    ...
    const confirmButton = page.locator('#submit');
    ...
    console.warn(`Teardown failed for user ${username}:`, error);
  }
}
