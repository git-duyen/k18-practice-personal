...
test('Personal Notes Test', async ({ page }) => {
    ...
    await test.step('Add 10 Notes', async () => {
        await personalNotePage.addNotes(page, notesData);
    });
    let randomNoteIndex = Math.floor(Math.random() * notesData.length);
    await test.step('Search theo keyword bất kì', async () => {
        await personalNotePage.searchNotesByKeyword(page, notesData[randomNoteIndex].title);
    });
    await test.step('Verify search results contains search keyword', async () => {
        await personalNotePage.verifySearchResults(page, notesData[randomNoteIndex].title);
    });
});
