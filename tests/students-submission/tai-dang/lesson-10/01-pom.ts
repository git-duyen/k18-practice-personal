...
export class RegisterPage extends MaterialBasePage {
    ...
    async fillUserName(page: Page, name: string) {
        await page.locator(this.xpathUserName).fill(name);
    }
    ...
    async clickSubmit(page: Page) {
        await page.locator(this.xpathRegisterButton).click();
    }

    async checkGender(page: Page, gender: string) {
        if (gender === 'Male') {
            await page.locator(this.xpathGenderMale).check();
        } else if (gender === 'Female') {
            await page.locator(this.xpathGenderFemale).check();
        }
    }

    async verifyTableData(page: Page, userName: string, email: string, gender: string) {
...
export class ProductPage extends MaterialBasePage {
    ...
    async addProductToCart(page: Page, productName: string, times: number) {
    ...
    async verifyTableData(page: Page, productName: string, quantity: string) {
        await expect(page.locator('table')).toBeVisible();
    ...
    async verifyTotal(page: Page, productName: string, quantity: string): Promise<number> {
        const row = page.locator('table tbody tr', { hasText: productName });
    ...
    async verifyTotalPrice(page: Page, expectedTotal: number) {
        const totalText = await page.locator('//td[@class="total-price"]').textContent();
        const total = parseFloat(totalText?.replace('$', '') || '0');
        expect(total).toBeCloseTo(expectedTotal, 2);
    }
}
...
export class PersonalNotePage extends MaterialBasePage {
    ...
    async addNotes(page: Page, notesToAdd: { title: string; content: string }[]) {
    ...
    async searchNotesByKeyword(page: Page, keyword: string) {
        const searchInput = page.locator('#search');
        await searchInput.fill(keyword);
    }

    // Verify search results
    async verifySearchResults(page: Page, expectedTitle: string) {
        let searchResults = await page.locator('#notes-list li div strong').textContent();
        expect(searchResults).toContain(expectedTitle);
    }
}
