...
test.describe("ACCOUNT-Account", () => {
    ...
    test("Create acc with editor permission", async ({ page }) => {
        ...
        await test.step("Add user", async () => {
            ...
            await page.locator("#user_login").fill(userToCleanup.userName);
            await page.locator("#email").fill(userToCleanup.email);
            await page.locator("#first_name").fill(userToCleanup.firstName);
            await page.locator("#last_name").fill(userToCleanup.lastName);
            await page.locator("#pass1").fill(userToCleanup.passWord);
            await page.locator("input[name='pw_weak']").check();
            await page.locator("#role").selectOption({ label: userToCleanup.role });
        ...
        await test.step("Logout and Login again with new acc", async () => {
            ...
            await page.locator("#wp-submit").click();
            //Check hiển thị menu
            await expect(page.locator("div.wp-menu-name:text('Dashboard')")).toBeVisible();
            await expect(page.locator("div.wp-menu-name:text('Profile')")).toBeVisible();
            await expect(page.locator("div.wp-menu-name:text('Appearance')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Users')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Plugins')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Posts')")).toBeVisible();
            await expect(page.locator("div.wp-menu-name:text('Media')")).toBeVisible();
            await expect(page.locator("div.wp-menu-name:text('Pages')")).toBeVisible();
            await expect(page.locator("//div[@class='wp-menu-name' and text()='Comments ']")).toBeVisible();
            // await expect(page.locator("div.wp-menu-name:text('Comments')")).toBeVisible({timeout :5_000});
            await expect(page.locator("div.wp-menu-name:text('Tools')")).toBeVisible();
        });
    });
    test("Create acc with subscriber permission", async ({ page }) => {
        ...
        await test.step("Logout and Login again with new acc", async () => {
            ...
            // await page.locator("//a[contains(text(), 'Log Out')]").click();
            await page.locator("#user_login").fill(userToCleanup.userName);
            await page.locator("#user_pass").fill(userToCleanup.passWord);
            await page.locator("#wp-submit").click();
            //Check hiển thị menu
            await expect(page.locator("div.wp-menu-name:text('Dashboard')")).toBeVisible();
            await expect(page.locator("div.wp-menu-name:text('Profile')")).toBeVisible();
            ...
            await expect(page.locator("div.wp-menu-name:text('Users')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Plugins')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Posts')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Media')")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Pages')")).toBeHidden();
            await expect(page.locator("//div[@class='wp-menu-name' and text()='Comments ']")).toBeHidden();
            await expect(page.locator("div.wp-menu-name:text('Tools')")).toBeHidden();
        });
    });
});